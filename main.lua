-- EL 01 ROX HUB VERSIÓN FINAL V7.4 - CÁMARA ESTIRADA REGRESADA
local Rayfield = loadstring(game:HttpGet('https://sirius.menu/rayfield'))()
local Window = Rayfield:CreateWindow({
   Name = "EL 01 ROX HUB",
   LoadingTitle = "EL 01 ROX HUB",
   LoadingSubtitle = "VERSIÓN FINAL V7.4",
...
AimTab:CreateSection("CÁMARA ESTIRADA - REGRESADO")
AimTab:CreateToggle({
    Name = "ESTIRAR CÁMARA",
    CurrentValue = false,
    Flag = "StretchFlag",
    Callback = function(v)
        _G.StretchEnabled = v
        if not v then
            Camera.FieldOfView = _G.NormalFOV
        else
            Camera.FieldOfView = _G.FOVValue
        end
        if v then
            Rayfield:Notify({Title="CÁMARA", Content="Cámara estirada activada", Duration=2})
        else
            Rayfield:Notify({Title="CÁMARA", Content="Cámara normal", Duration=2})
        end
    end
})
AimTab:CreateSlider({
    Name = "Cuánto lo quieres estirar",
    Range = {70, 120},
    Increment = 1,
    CurrentValue = 90,
    Flag = "FOVValueFlag",
...
CreditTab:CreateParagraph({Title = "EL 01 ROX HUB", Content = "VERSIÓN FINAL V7.4\nTodo arreglado\nCreado por EL 01 ROX HUB"})
